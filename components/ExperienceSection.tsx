...
export default function ExperienceSection() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-32 sm:px-12 relative font-sans" ref={containerRef}>
      ...
      <div className="mb-24 md:mb-32 relative" data-speed="1.1">
        <div className="flex items-center gap-3 font-mono text-xs text-signal-cyan uppercase tracking-widest mb-4">
          <span className="h-2 w-2 rounded-full bg-signal-cyan animate-pulse shadow-[0_0_10px_#00F0FF]" />
          <span>// CHRONO_LOGS</span>
        </div>
        <h2 className="font-display text-5xl md:text-7xl font-black uppercase tracking-tighter text-white mb-6 relative z-10">
      ...
      <div className="relative flex flex-col gap-32">
        {EXPERIENCES.map((exp, idx) => {
          ...
          return (
            <div 
              key={idx} 
              className={`exp-item relative flex flex-col ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-8 md:gap-16 group`}
            >
              ...
              <div 
                className="w-full md:w-2/3 hud-card bg-[#0A0A0F]/80 backdrop-blur-xl border border-white/10 p-8 md:p-10 transition-all duration-500 hover:shadow-[0_0_40px_rgba(0,240,255,0.15)] hover:border-signal-cyan/50 relative overflow-hidden"
              ...
              >
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8 border-b border-white/10 pb-6 relative z-10">
                  <div>
                    <h3 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight group-hover:text-signal-cyan transition-colors mb-2 font-display">
                      {exp.role}
                    </h3>
                  ...
                  <div className="font-mono text-[11px] text-signal-pink bg-signal-pink/10 border border-signal-pink/30 px-3 py-1.5 self-start font-bold whitespace-nowrap">
                ...
                <ul className="space-y-4 mb-8 relative z-10">
                  {exp.points.map((point, pIdx) => (
                    <li key={pIdx} className="text-gray-300 font-sans text-sm md:text-base leading-relaxed flex items-start gap-4">
                      <span className="text-signal-cyan font-mono mt-1 text-[10px] select-none">■</span>
                      <span>
                ...
                <div className="flex flex-wrap gap-2 pt-4 border-t border-white/5 relative z-10">
                  {exp.techStack.map((tech, tIdx) => (
                    <span 
                      key={tIdx} 
                      className="font-mono text-[10px] tracking-widest text-gray-400 bg-white/[0.02] border border-white/10 px-3 py-1.5 hover:border-signal-cyan hover:text-signal-cyan transition-colors"
                    >
                      {tech.toUpperCase()}
                    </span>
                  ))}
